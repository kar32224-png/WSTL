...
const sections = [
  { id: "hero", label: "Home" },
  { id: "projects", label: "Offerings" },
  { id: "services", label: "Approach" },
...
const menuItems = [
  ...
  { label: "Approach", href: "#services-menu" },
  ...
  return (
    <AnimatePresence>
      ...
      <div className="mx-auto flex max-w-360 items-center justify-between gap-4 2xl:max-w-450 3xl:max-w-550">
        <motion.a
          href="/"
          className={`flex h-12 sm:h-16 items-center justify-center rounded-xl sm:rounded-2xl backdrop-blur-lg px-3 sm:px-4 shadow-lg shrink-0 ${isDark ? "bg-black" : "bg-white"}`}
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <Image
            src="/faxis/faxis-logo.png"
            alt="FAxis"
            width={40}
            height={40}
            className={`h-9 sm:h-12 w-auto object-contain ${isDark ? "brightness-0 invert" : ""}`}
            priority
          />
        </motion.a>
